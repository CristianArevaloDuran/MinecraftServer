...
public final class CustomData {
    ...
    public static final Codec<CustomData> CODEC = NBTTagCompound.CODEC.xmap(CustomData::new, (customdata) -> {
    ...
    public <T> DataResult<CustomData> update(MapEncoder<T> mapencoder, T t0) {
        return mapencoder.encode(t0, DynamicOpsNBT.INSTANCE, DynamicOpsNBT.INSTANCE.mapBuilder()).build(this.tag).map((nbtbase) -> {
            return new CustomData((NBTTagCompound) nbtbase);
        });
    }

    public <T> DataResult<T> read(MapDecoder<T> mapdecoder) {
        MapLike<NBTBase> maplike = (MapLike) DynamicOpsNBT.INSTANCE.getMap((NBTBase) this.tag).getOrThrow();

        return mapdecoder.decode(DynamicOpsNBT.INSTANCE, maplike);
    }
